...
namespace ufo {
  ...
  static ObsFunctionMaker<ObsStringFormatter> makerObsStringFormatter_("ObsStringFormatter");
  ...
  static void format_var_type(std::vector<boost::format> &fmt, const ObsFilterData &obs,
                              const Variable &var) {
    std::vector<ToType> mutableData;
    obs.get(var, mutableData);

    const std::vector<ToType> &constData = mutableData;
  ...
  static void format_one_var(std::vector<boost::format> &fmt, const ObsFilterData &obs,
    ...
    if (targetType == ioda::ObsDtype::DateTime && optionalDateFormat) {
      ...
      std::vector<ObsTypedef::DateTime> mutableData;
      obs.get(var, mutableData);

      const std::vector<ObsTypedef::DateTime> & constData = mutableData;
      const size_t nlocs = obs.nlocs();
      for (size_t i = 0; i < nlocs; i++)
        fmt[i] % constData[i].formatString(dateFormat);
  ...
  void ObsStringFormatter::compute(const ObsFilterData & in,
    ...
    // For each variable, loop over all observations, and give them to the format object.
    for (const auto &param : variables)
      format_one_var(fmt, in, param.variable.value(), param.dateFormat.value());

    // Output each format object as a string to ``out[0]`` via an ostringstream.
    for (size_t i = 0; i < nlocs; i++)
